library(dplyr)

flight_url <- function(year, month) {
  base_url <- "http://www.transtats.bts.gov//PREZIP/"
  sprintf(paste0(base_url, "On_Time_On_Time_Performance_%d_%d.zip"), year, month)
}

download_month <- function(year, month) {
  url <- flight_url(year, month)

  temp <- tempfile(fileext = ".zip")
  download.file(url, temp)

  files <- unzip(temp, list = TRUE)
  # Only extract biggest file
  csv <- files$Name[order(files$Length, decreasing = TRUE)[1]]

  unzip(temp, exdir = "data-raw/flights", junkpaths = TRUE, files = csv)

  src <- paste0("data-raw/flights/", csv)
  dst <- paste0("data-raw/flights/", year, "-", month, ".csv")
  file.rename(src, dst)
}

months16 <- 6:12
months17 <- 1:5
needed16 <- paste0("2016-", months16, ".csv")
needed17 <- paste0("2017-", months17, ".csv")
missing16 <- months16[!(needed16 %in% dir("data-raw/flights"))]
missing17 <- months17[!(needed17 %in% dir("data-raw/flights"))]

lapply(missing16, download_month, year = 2016)
lapply(missing17, download_month, year = 2017)

get_pnw <- function(path) {
  read.csv(path, stringsAsFactors = FALSE) %>%
    tbl_df() %>%
    select(
      year = Year, month = Month, day = DayofMonth, dep_time = DepTime,
      dep_delay = DepDelay, arr_time = ArrTime, arr_delay = ArrDelay,
      carrier = Carrier,  tailnum = TailNum, flight = FlightNum,
      origin = Origin, dest = Dest, air_time = AirTime, distance = Distance
    ) %>%
    mutate(hour = dep_time %/% 100, minute = dep_time %% 100) %>%
    filter(origin %in% c("PDX", "SEA")) %>%
    arrange(year, month, day, dep_time)
}

all <- lapply(dir("data-raw/flights", full.names = TRUE), get_pnw)
flights <- rbind_all(all) %>% tbl_df()

save(flights, file = "data/flights.rda", compress = "bzip2")

# Save to RDS file
saveRDS(flights, "data/flights.RDS")

# Export to CSV
write.csv(flights, "data/flights.csv", row.names = FALSE)
